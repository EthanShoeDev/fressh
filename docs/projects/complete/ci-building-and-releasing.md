...
| `sync-listing:ios` | `ios sync_listing` | `deliver` metadata + screenshots → product page, **no** binary upload, **no** review submit (auto-runs after each iOS beta in `build-mobile.yml`) |
...
- **Secrets** (env via secretspec): `GOOGLE_SERVICE_ACCOUNT_KEY_JSON` (supply `json_key_data` — no
  ...
  `ASC_REVIEW_CONTACT_PHONE` (App Review contact phone — secret because the repo is public; the
  contact name/email + demo notes are non-secret and inline in the Fastfile).
- **Store listings as code:** `apps/mobile/fastlane/metadata/` (iOS locale dirs + app-level
  `copyright`/`primary_category`/`secondary_category` + `android/en-US/`)
  and `fastlane/screenshots/` are the listings — edit in git, `deliver`/`supply` push them. iOS pushes
  via `ios sync_listing` (`deliver`, no binary, no submit), which **auto-runs after every iOS beta**
  so the App Store product page stays in sync; `ios release` reuses the same push and additionally
  submits for review. **App Review contact info** is passed inline (`app_review_information`, phone
  from the `ASC_REVIEW_CONTACT_PHONE` secret) on *every* push, not just submit: deliver's
  `review_attachment_file` step unconditionally fetches the review detail and dies with "No data" if
  it was never created (fastlane #20538), and `review_information` is what creates it. The iOS
  **app icon is NOT pushable** (deliver dropped it in the 2020 ASC-API
  migration) — it comes from the build's asset catalog (expo prebuild generates the 1024 marketing
  icon), so the product-page icon appears once a build is *attached* to the version (sync attaches the
  latest processed build locally; CI skips attach via `FRESSH_SYNC_ATTACH_BUILD=false` because the
  just-uploaded build is still processing — it's attached at promote time instead).
  ⚠️ **iOS store screenshots must be 6.9"** (iPhone 16 Pro Max, 1320×2868) — Apple's required size,
  auto-scaled to smaller devices. `scripts/screenshots.ts` warns if the booted sim is smaller (a 6.3"
  "iPhone 16 Pro" yields 1206×2622, which deliver files under the optional 6.1" slot).
...
7. iOS listing text + screenshots: `fastlane/metadata/en-US/` + `fastlane/metadata/` (categories,
   copyright) + `fastlane/screenshots/en-US/` — deliver pushes them via `sync-listing:ios` (auto-runs
   after each iOS beta; `promote:ios` also submits for review). App Review contact info is inline in
   the Fastfile (phone from the `ASC_REVIEW_CONTACT_PHONE` secret), not files.
