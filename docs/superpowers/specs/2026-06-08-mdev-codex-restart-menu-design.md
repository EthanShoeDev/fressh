...
- Add `restart codex`, which runs `mdev codex restart`.
...
| `restart codex` | text `mdev codex restart`, then Enter |

The removed `codex auth refresh new` label should not appear in the bundled
menu tree.

## Data Flow

The command menu continues to load from
`apps/mobile/config/shell-config.json`. Selecting either Codex preset inserts
the configured text into the active shell and sends Enter through the existing
command preset runner.

## Error Handling

No new app-side error handling is needed. If `mdev codex restart` fails, the
remote shell shows the command output using the existing preset execution path.
...
- `restart codex` sends `mdev codex restart`.
