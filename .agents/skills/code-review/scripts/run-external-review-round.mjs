...
function resolveCodexSandboxBackend() {
	const override = String(
		process.env[CODEX_SANDBOX_BACKEND_ENV] ?? CODEX_SANDBOX_BACKEND_AUTO,
	)
		.trim()
		.toLowerCase();
...
function buildCodexReviewScopeArgs(reviewTarget) {
	...
	const parsedTarget = parseNormalizedReviewTarget(normalizedTarget);
...
export function buildCodexReviewCommand(reviewTarget, options = {}) {
	...
	const { kind, scopeArgs, supportsProfilePrompt, targetLabel } =
		buildCodexReviewScopeArgs(reviewTarget);
	const { commandPrefix } = buildCodexCommandPrefix();
	const needsPromptPipe = Boolean(reviewProfile) || kind === 'literal';

	if (!needsPromptPipe) {
		return `${commandPrefix} review ${scopeArgs}`;
	}
	if (kind === 'literal' && !reviewInputFile) {
		throw new Error(
			'Literal review targets require a prepared review input file',
		);
	}

	const resolvedReviewProfile = resolveReviewProfileDefinition(
		reviewProfile ?? DEFAULT_REVIEW_PROFILE,
	);
	if (!supportsProfilePrompt) {
		throw new Error(
			'Codex review profiles require an explicit review target flag like --uncommitted, --base, or --commit',
		);
	}
	...
	const renderPromptCommand = [
		...
		...(targetLabel
			? ['--review-target-label', shellQuoteArg(targetLabel)]
			: []),
		...(reviewInputFile
			? ['--review-input-file', shellQuoteArg(reviewInputFile)]
			: []),
...
function normalizeStoredReviewProvider(provider) {
	const normalizedProvider = String(provider ?? CODEX_PROVIDER)
		.trim()
		.toLowerCase();
...
export function classifyExistingRoundState(steps, { round, provider }) {
	...
	const terminalStep =
		[...matchingSteps].reverse().find((step) => step.status !== 'started') ??
		null;
...
async function loadInitializedRunState(runDir, repoRoot) {
	...
	const metadataEvent = events.find(
		(event) => event.type === 'run_initialized',
	);
...
async function resumeActiveExternalReviewRound({
	...
	const parsed =
		parsedLog.state === 'unknown'
			? parseCodexReviewText(fullLog ?? '')
			: parsedLog;

	if (
		exitCode !== null &&
		(parsed.state === 'clean' || parsed.state === 'issues_found')
	) {
		return {
			state: parsed.state,
	...
	return buildInProgressRoundResult({
		...
		reviewId,
		activeStartedStep,
		summary: `Round ${activeStartedStep.round} Codex external review is already active; keep waiting for the existing review to finish.`,
		reviewLogPath,
	});
}

async function resolveTerminalArtifactDestination({
	repoRoot,
	runDir,
	round,
	state,
	...
	const runRoot = path.isAbsolute(runDir)
		? runDir
		: path.join(repoRoot, runDir);
	...
	throw new Error(
		`Could not allocate a unique ${state} artifact path for round ${round}`,
	);
}

async function writePriorReviewedContextFile({
	absoluteRunDir,
	round,
	context,
}) {
...
async function writePreparedReviewInputFile({
	absoluteRunDir,
	round,
	reviewInput,
}) {
	...
	const filePath = path.join(
		...
		'rounds',
		String(round),
		'prepared-review-input.md',
	);
	await mkdir(path.dirname(filePath), { recursive: true });
	await writeFile(filePath, reviewInput, 'utf8');
...
export async function runExternalReviewRound({
	...
	const { reviewLogPath, reviewPidFilePath, reviewExitFilePath } =
		buildTempPaths(validatedReviewId, validatedRound);
	const {
		absoluteRunDir,
	...
	if (!resumedReviewScope) {
		...
		const effectiveTarget = parseNormalizedReviewTarget(
			resolvedReviewScope.effectiveReviewTarget,
		);
		...
		if (effectiveTarget.kind === 'literal') {
			reviewInputFile = await writePreparedReviewInputFile({
				absoluteRunDir,
				round: validatedRound,
				reviewInput,
			});
	...
	const command =
		reviewCommand ??
		existingRoundState.activeStartedStep?.command ??
...
export function parseCliArgs(argv = process.argv.slice(2)) {
	...
	return {
		...
		stallMs: ensurePositiveInteger(
			values['stall-ms'],
			'stall-ms',
			DEFAULT_STALL_MS,
		),
		pollMs: ensurePositiveInteger(
			values['poll-ms'],
			'poll-ms',
			DEFAULT_POLL_MS,
		),
...
function printUsage() {
	process.stdout.write(
		[
			...
			'    --repo-root <path> --run-dir <path> --round <n> --review-id <id> [--review-mode auto-detect|uncommitted|branch-stack] [--base-ref <ref>] [--review-target --uncommitted|--base <ref>|--pr <base>|--commit <sha>] [--review-profile default|mix|roasted|architect|correctness]',
			'',
		].join('\n'),
	);
}
