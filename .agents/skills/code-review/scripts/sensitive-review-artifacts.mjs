...
async function collectNameOnlyPaths(
	...
	return entries.map((entry) => normalizeRelativePath(entry)).filter(Boolean);
}
...
function parseNameStatusPaths(stdout) {
	...
	for (let index = 0; index < entries.length; ) {
		...
		for (
			let count = 0;
			count < pathCount && index < entries.length;
			count += 1
		) {
...
async function collectWorktreeOverlayPaths(
	...
	return [
		...new Set([...unstagedPaths, ...stagedPaths, ...untrackedPaths]),
	].sort();
}
...
export async function findSensitiveReviewArtifacts({
	...
	if (normalizedTarget.kind === 'uncommitted') {
		addBlockedPaths(
			await collectWorktreeOverlayPaths(repoRoot, {
	...
	if (normalizedTarget.kind === 'literal') {
		addBlockedPaths([normalizedTarget.value]);
		addBlockedPaths(
			await collectNameStatusPaths(repoRoot, [
				'diff',
				'--name-status',
