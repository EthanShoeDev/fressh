...
import { canonicalizeReviewTarget, parseNormalizedReviewTarget } from "./review-target.mjs";
...
async function buildUncommittedReviewInput(
  ...
  const excludedPathspecs = buildExcludedPathspecs({ excludeCodeReviewArtifacts });
  const [unstagedDiff, stagedDiff, untrackedFiles] = await Promise.all([
    runGit(
      repoRoot,
      appendPathspecArgs(["diff", "--find-renames", "--binary"], excludedPathspecs)
    ),
    runGit(
      repoRoot,
      appendPathspecArgs(
...
export async function prepareReviewInput({
  ...
  if (normalizedTarget.kind === "uncommitted") {
    ...
    reviewInput = await buildCommitReviewInput(repoRoot, normalizedTarget.value);
    ...
    reviewInput = await buildPullRequestReviewInput(repoRoot, normalizedTarget.value, {
      excludeCodeReviewArtifacts,
    });
  } else {
    throw new Error(`Unsupported normalized review target: ${normalizedTarget.kind}`);
  }
...
export function parseCliArgs(argv = process.argv.slice(2)) {
  ...
  return {
    ...
    excludeCodeReviewArtifacts: values["exclude-code-review-artifacts"] ?? false,
  };
}
