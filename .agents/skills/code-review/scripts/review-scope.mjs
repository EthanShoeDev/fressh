...
export function normalizeReviewModeValue(reviewMode, defaultValue = null) {
	const normalizedReviewMode = String(reviewMode ?? '')
		.trim()
		.toLowerCase();
...
async function runGit(repoRoot, args, { env = {}, trim = true } = {}) {
...
function buildLiteralTargetPolicy(reviewTarget) {
	...
	const parsedReviewTarget = parseNormalizedReviewTarget(
	...
	if (parsedReviewTarget.kind === 'base') {
		return {
			targetKind: parsedReviewTarget.kind,
...
export async function resolveInitialReviewPolicy({
	...
	if (normalizedReviewMode === 'uncommitted') {
		if (
			...
			literalTargetPolicy.targetKind !== 'literal'
		) {
			throw new Error(
		...
		if (literalTargetPolicy?.targetKind === 'literal') {
			return {
				...literalTargetPolicy,
				reviewMode: 'uncommitted',
			};
...
export async function createBranchStackSnapshot({
	...
	if (!Number.isFinite(resolvedRound) || resolvedRound <= 0) {
		throw new Error(
			'round must be a positive integer for branch-stack snapshots',
		);
	}
	...
	const mergeBaseCommit = await resolveMergeBaseCommit(
		repoRoot,
		resolvedBaseRef,
	);
	const tempDir = await mkdtemp(
		path.join(
			os.tmpdir(),
			`code-review-${normalizedReviewId}-r${resolvedRound}-`,
		),
	);
